...
class Menu:
    def __init__(self, stdscr, title="Menu"):
        ...
        self.__title_y = 1
        self.__title_x = self.__get_middle_x(self.__title)  # Center the title

        self.__greet_title = "ATM Menu"
        self.__greet_title_y = 1
        self.__greet_title_x = self.__get_middle_x(
            self.__greet_title
        )  # Center the greet title

        # Initialize the menu options
        self.__menu_actions = {
            "d": {
                "msg": "Press button d to Deposit Money",
                "function": self.__deposit_screen,
            },
            "w": {
                "msg": "Press button w to Withdraw Money",
                "function": self.__withdraw_screen,
            },
            "c": {
                "msg": "Press button c to Check your Balance",
                "function": self.__balance_screen,
            },
            "q": {"msg": "Press to q to Quit", "function": self.__logout},
            "p": {
                "msg": "Press button p to Change PIN_CODE",
                "function": self.__change_pin_screen,
            },
            "r": {
                "msg": "Press button r to Get a RECIPE",
                "function": self.__recipe_screen,
            },
        }
        ...
        * -> (option: amount) ~ (str: int[amount] / str["Other"])
        ...
        self.__withdraw_options = {
            str(option + 1): (
                self.__withdraw_amounts[option]
                if option < len(self.__withdraw_amounts)
                else "Other"
            )
    ...
    def __display_title(self):
        ...
        self.stdscr.addstr(
            ...
            self.__title_x,
            self.__title,
            self.__terminal_color | curses.A_BOLD,
        )
    # END __display_title
    ...
    def __display_greet_title(self, user_fullname):
        self.stdscr.addstr(
            self.__greet_title_y,
            self.__greet_title_x,
            self.__greet_title,
            self.__terminal_color | curses.A_BOLD,
        )
    ...
    def __show_deposit_inputbox(self, desposit_y, formerElement=False) -> int:
        ...
        deposit_win = curses.newwin(
            1, max_deposit_length, desposit_y, deposit_x + len(deposit_prompt)
        )
        curses.textpad.rectangle(
            self.stdscr,
            desposit_y - 1,
            deposit_x + len(deposit_prompt) - 1,
            desposit_y + 1,
            deposit_x + len(deposit_prompt) + max_deposit_length,
        )
        self.stdscr.refresh()
        ...
        while True:
            ...
            # END elif
                ...
                if len(deposit_amount) == 0:
                    ...
                    self.stdscr.addstr(
                        desposit_y + 2,
                        self.__get_middle_x(empty_deposit_amount_msg),
                        empty_deposit_amount_msg,
                        self.__terminal_color
                        | curses.A_BOLD
                        | curses.A_STANDOUT
                        | curses.A_UNDERLINE,
                    )
    ...
    def __show_withdraw_options(self, withdraw_y, formerElement=False) -> int:
        ...
        # Holder for other amount to be withdrawed
        other_amount = ""
        ...
        while not selection_done:
            ...
            if chr(chosen_option) in self.__withdraw_options.keys():
                if isinstance(
                    self.__withdraw_options[chr(chosen_option)], int
                ):  # Means it's a number of NIS
                ...
                # END if
                    ...
                    other_win = curses.newwin(
                        1,
                        max_other_amount_length,
                        other_y,
                        other_x + len(other_amount_prompt),
                    )
                    curses.textpad.rectangle(
                        self.stdscr,
                        other_y - 1,
                        other_x + len(other_amount_prompt) - 1,
                        other_y + 1,
                        other_x
                        + len(other_amount_prompt)
                        + max_other_amount_length
                        + 1,
                    )
                    self.stdscr.refresh()
                    while True:
                        ...
                        if key in range(
                            48, 58
                        ):  # Check if the key is a digit (ASCII 48-57)
                            if (
                                len(other_amount) == 0 and chr(key) == "0"
                            ):  # disallow "0" as a first char of amount
                                continue
                            # END if
    ...
    def __show_user_id_num_inputbox(self, user_id_num_y, formerElement=False) -> str:
        ...
        user_id_num_x = self.__get_middle_x(
            user_id_num_prompt + " " * max_user_id_num_length
        )
        # Username input box
        self.stdscr.addstr(
            user_id_num_y, user_id_num_x, user_id_num_prompt, self.__input_color
        )
        user_id_num_win = curses.newwin(
            1,
            max_user_id_num_length,
            user_id_num_y,
            user_id_num_x + len(user_id_num_prompt),
        )
        curses.textpad.rectangle(
            self.stdscr,
            user_id_num_y - 1,
            user_id_num_x + len(user_id_num_prompt) - 1,
            user_id_num_y + 1,
            user_id_num_x + len(user_id_num_prompt) + max_user_id_num_length,
        )
        user_id_num = ""
        ...
        while True:
            ...
            if key in range(48, 58) and len(user_id_num) < (
                max_user_id_num_length - 1
            ):  # Check if the key is a digit (ASCII 48-57)
            ...
            # END if
                ...
                self.stdscr.addstr(
                    user_id_num_y + 2,
                    self.__get_middle_x(len_9_user_id_num_msg),
                    len_9_user_id_num_msg,
                    self.__terminal_color
                    | curses.A_BOLD
                    | curses.A_STANDOUT
                    | curses.A_UNDERLINE,
        ...
        # END while
        self.__clear_line(
            *range(user_id_num_y - 1, user_id_num_y + 2 + 1)
        )  # Clear textbox & username input line
        welcome_msg = f"Welcome, {'*' * (max_censored_length)}{user_id_num[max_censored_length:]}!"
        welcome_x = self.__get_middle_x(welcome_msg)
        self.stdscr.addstr(
            user_id_num_y, welcome_x, welcome_msg, self.__terminal_color | curses.A_BOLD
        )
        self.stdscr.refresh()
    ...
    def __show_username_inputbox(self, username_y, formerElement=False) -> str:
        ...
        username_win = curses.newwin(
            1, max_username_length, username_y, username_x + len(username_prompt)
        )
        curses.textpad.rectangle(
            self.stdscr,
            username_y - 1,
            username_x + len(username_prompt) - 1,
            username_y + 1,
            username_x + len(username_prompt) + max_username_length,
        )
        username = ""
        # Input for username
        while len(username) == 0:
            ...
            if len(username) == 0:
                ...
                self.stdscr.addstr(
                    username_y + 2,
                    self.__get_middle_x(empty_username_msg),
                    empty_username_msg,
                    self.__terminal_color
                    | curses.A_BOLD
                    | curses.A_STANDOUT
        ...
        # END while
        self.__clear_line(
            *range(username_y - 1, username_y + 2 + 1)
        )  # Clear textbox & username input line
        welcome_msg = f"Welcome, {username}!"
        welcome_x = self.__get_middle_x(welcome_msg)
        self.stdscr.addstr(
            username_y, welcome_x, welcome_msg, self.__terminal_color | curses.A_BOLD
        )
        self.stdscr.refresh()
    ...
    def __show_pincode_inputbox(self, pin_y, formerElement=False) -> str:
        ...
        curses.textpad.rectangle(
            self.stdscr,
            pin_y - 1,
            pin_x + len(pin_prompt) - 1,
            pin_y + 1,
            pin_x + len(pin_prompt) + max_pin_length,
        )
        self.stdscr.refresh()
        # PIN input
        pin = ""
        while True:
            ...
            # END elif
                ...
                self.stdscr.addstr(
                    pin_y + 2,
                    self.__get_middle_x(empty_pin_msg),
                    empty_pin_msg,
                    self.__terminal_color
                    | curses.A_BOLD
                    | curses.A_STANDOUT
    ...
    def __display_back_title(self):
        ...
        self.stdscr.addstr(
            self.height - 1,
            self.__get_middle_x(get_back_msg),
            get_back_msg,
            self.__terminal_color | curses.A_BOLD | curses.A_STANDOUT,
        )
        self.stdscr.refresh()
    ...
    def __deposit_screen(self):
        ...
        middle_y = (
            self.height // 2
        ) - 3  # minus 3 because we have 3 lines to show (deposit_msg | error_msg), the input box, and one empty break line between them
        try:
            self.stdscr.clear()
            deposit_msg = "How much would you like to Deposit?"
            self.stdscr.addstr(
                middle_y,
                self.__get_middle_x(deposit_msg),
                deposit_msg,
                self.__terminal_color | curses.A_BOLD | curses.A_STANDOUT,
            )
            self.stdscr.refresh()
            ...
            self.stdscr.addstr(
                middle_y,
                self.__get_middle_x(deposit_success_msg),
                deposit_success_msg,
                self.__terminal_color | curses.A_BOLD | curses.A_STANDOUT,
            )
            self.stdscr.refresh()
        ...
        # END except
        middle_y += (
            2  # 1 line for new pin msg + input box takes 2 lines + 1 empty break line
        )
        deposit_exit_msg = "Press any key to get BACK"
        self.stdscr.addstr(
            middle_y,
            self.__get_middle_x(deposit_exit_msg),
            deposit_exit_msg,
            self.__terminal_color | curses.A_BOLD | curses.A_STANDOUT,
        )
        self.stdscr.refresh()
    ...
    def __withdraw_screen(self):
        ...
        middle_y = (self.height // 2) - (
            withdraw_options_lines_amount + 2
        )  # amount of withdraw options is the amount of rows needed + 1 for the withdraw_msg + 1 empty break line
        try:
            self.stdscr.clear()
            withdraw_msg = "How much would you like to Withdraw?"
            self.stdscr.addstr(
                middle_y,
                self.__get_middle_x(withdraw_msg),
                withdraw_msg,
                self.__terminal_color | curses.A_BOLD,
            )
            middle_y += 2
            ...
            self.stdscr.addstr(
                middle_y,
                self.__get_middle_x(withdraw_success_msg),
    ...
    def __balance_screen(self):
        # __atm.get_balance
        self.stdscr.clear()
        balance_msg = ["Your current balance: ", f"{self.__atm.get_balance()} NIS"]
        ...
        for msg in balance_msg:
            self.stdscr.addstr(
                ...
                self.__get_middle_x(msg),
                msg,
                self.__terminal_color | curses.A_BOLD | curses.A_ITALIC,
            )
            middle_y += 1
        # END for
        ...
        self.stdscr.addstr(
            middle_y + 1,
            self.__get_middle_x(balance_exit_msg),
            balance_exit_msg,
            self.__terminal_color | curses.A_BOLD | curses.A_STANDOUT,
        )
        self.stdscr.refresh()
    ...
    def __logout(self):
        middle_y = (
            self.height // 2
        ) - 2  # minus 2 because we have 2 lines to show (logout_msg | error_msg), and one empty break line between them
        try:
            ...
            self.stdscr.addstr(
                middle_y,
                self.__get_middle_x(logout_msg),
                logout_msg,
                self.__terminal_color | curses.A_BOLD | curses.A_STANDOUT,
            )
            self.stdscr.refresh()
        ...
        self.stdscr.addstr(
            middle_y + 2,
            self.__get_middle_x(logout_exit_msg),
            logout_exit_msg,
            self.__terminal_color | curses.A_BOLD | curses.A_STANDOUT,
        )
        self.stdscr.refresh()
    ...
    def __change_pin_screen(self):
        middle_y = (
            self.height // 2
        ) - 3  # minus 3 because we have 3 lines to show (new_pin_msg | error_msg), the input box, and one empty break line between them
        try:
            self.stdscr.clear()
            new_pin_msg = f"Hello {str(self.__atm.get_user_name())}, Enter NEW PINCODE"
            self.stdscr.addstr(
                middle_y,
                self.__get_middle_x(new_pin_msg),
                new_pin_msg,
                self.__terminal_color | curses.A_BOLD | curses.A_STANDOUT,
            )
            self.stdscr.refresh()
        ...
        # END except
        middle_y += (
            4  # 1 line for new pin msg + input box takes 2 lines + 1 empty break line
        )
        new_pin_exit_msg = "PINCODE changed succesfuly! Press any key to get BACK"
        self.stdscr.addstr(
            middle_y + 2,
            self.__get_middle_x(new_pin_exit_msg),
            new_pin_exit_msg,
            self.__terminal_color | curses.A_BOLD | curses.A_STANDOUT,
        )
        self.stdscr.refresh()
    ...
    def __recipe_screen(self):
        ...
        self.stdscr.addstr(
            middle_y + 1,
            self.__get_middle_x(recipe_exit_msg),
            recipe_exit_msg,
            self.__terminal_color | curses.A_BOLD | curses.A_STANDOUT,
        )
        self.stdscr.refresh()
