...
class Atm:
    ...
    def login(self, user_id_num: str, pin: str) -> bool:
        ...
        try:
            ...
            with self.__db_config.connect() as (conn, cursor):
                # Step 1: SELECT user by id_number
                cursor.execute(
                    """
                    SELECT\
                        users.id, \
                    ...
                    FROM users
                    JOIN pincodes \
                        ON users.id = pincodes.user_id
                    WHERE users.id_number = %s
                """,
                    (user_id_num,),
                )
                # Handle duplicates with fetchall()
    ...
    """
        Raises a ValueError if the user is not logged in.
        Continues regularly if the user log out successful,
        and sets all info to None.
    """
    ...
    def is_valid_amount(self, amount: float) -> bool:
        return (
            ...
            if (
                amount > 0
                and (amount % 20 == 0 or amount % 50 == 0 or amount % 100 == 0)
            )
            else False
        )
    ...
    def set_user_name(self, fullName: str) -> str:
